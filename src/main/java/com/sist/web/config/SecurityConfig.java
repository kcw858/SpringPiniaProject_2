 ...
 *    Authentication : 인증된 사용자가 사이트에 접근가능한지 권한 확인
 *    
 *    저장(인증 = 권한 = 저장 = Session)
 *    				  ---
 *    				서버 종료시 메모리 해제
 ...
 *     	  - DataBase 연동 => 5) UserDetailService: 결과값 return
 *     			|
 *     		  4-1) User비교 / 암호
 *     					passwordEncoding
 *     
...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception
	{
		 ...
		 *   - 공격자가 인증된 브라우저에서 저장된 쿠키나 세션정보를 활용해서 다른 사이트로 요청값을 전송 (위조)
		 *   - 일반 보안: csrf.disable()
		 *   
		 */
		http
		...
		.logout(logout -> logout 
			  ...
			  .deleteCookies("remember-me","JSESSEIONID")
		);
		// remember-me
		return http.build();
	}
